rm(list = ls())
setwd("/home/ruiqliu/MPB/v2/linear_sigma")
options(width = 200)
...
#df=result
colnames(df)=c("omega","lambda","Iv","a","b1","b2","b3","sda", "sd1", "sd2", "sd3","n","seed")
...
re=c()
for (n in n.sim){
  subdf=df[which(df$n==n & df$omega!=0),]
  #avg=aggregate(cbind(subdf$error,subdf$Ivmax,subdf$Iv), list(subdf$lambda),FUN=mean)
  avg=aggregate(cbind(error,checkb1, checkb2, checkb3, Iv)~n+omega,data=subdf, FUN="mean")
...
re=re[order(re$n, -re$omega),]
re=re[,c(2,4,3,5,6,7,8)]