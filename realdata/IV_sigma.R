...
rho=function(theta,lambda=1){
  ...
  s=s*dnorm(theta[5],0,lambda)
  s=s*dnorm(theta[6],0,lambda)
 # s=s*dnorm(theta[7],0,lambda)
  ...
  s=s/dunif(theta[5],-srange,srange)
  s=s/dunif(theta[6],-srange,srange)
 # s=s/dunif(theta[7],-srange,srange)
  return(s)
}
